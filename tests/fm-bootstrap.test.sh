...
array use with all fanout is accepted^{"rules":[{"when":"research","use":[{"harness":"pi","model":"xai/grok","effort":"high"},{"harness":"pi","model":"openai/gpt","effort":"high"}],"select":"all"}]}^grep^  rule: research -> all[pi/xai/grok/high, pi/openai/gpt/high]
all fanout quota fallback is accepted^{"rules":[{"when":"research","use":[{"harness":"pi","model":"claude-bridge/claude-fable-5","effort":"max","quota":{"provider":"claude","window":"model:fable","percentRemainingAbove":0,"fallback":{"harness":"pi","model":"claude-bridge/claude-opus-4-8","effort":"max"}}}],"select":"all"}]}^grep^  rule: research -> all[pi/claude-bridge/claude-fable-5/max{quota:claude/model:fable>0 fallback:pi/claude-bridge/claude-opus-4-8/max}]
all fanout with object use is flagged^{"rules":[{"when":"research","use":{"harness":"pi"},"select":"all"}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - select all requires an array use
quota guard outside all is flagged^{"rules":[{"when":"research","use":{"harness":"pi","quota":{"provider":"claude","window":"model:fable","percentRemainingAbove":0,"fallback":{"harness":"pi"}}}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - quota guards require select all
quota guard without fallback is flagged^{"rules":[{"when":"research","use":[{"harness":"pi","quota":{"provider":"claude","window":"model:fable","percentRemainingAbove":0}}],"select":"all"}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - profile quota needs fallback profile
unverified quota fallback harness is flagged^{"rules":[{"when":"research","use":[{"harness":"pi","quota":{"provider":"claude","window":"model:fable","percentRemainingAbove":0,"fallback":{"harness":"spaceship"}}}],"select":"all"}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - unverified harness: spaceship
