...
[ -f "$CI" ] || CI="$ROOT/.github/workflows/integrity.yml"
...
test_ci_installs_and_logs_the_pinned_version() {
  ...
  local expected_installs=2
  [ "${CI##*/}" != integrity.yml ] || expected_installs=1
  [ "$(grep -Fc "bin/fm-install-shellcheck.sh \"\$RUNNER_TEMP/bin\"" "$CI")" -eq "$expected_installs" ] \
    || fail "every CI lint surface must use the shared ShellCheck installer"
